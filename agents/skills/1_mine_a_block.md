...
- Memorize block name
- if you need to move to the block to mine it, use `goto_block` (NOT `goto_position`) to navigate to the block's location. Wait until you receive `[SYSTEM EVENT: pathfinding_result]` before proceeding.
- Once you receive the pathfinding success event, use `start_dig` to begin mining the block.
...
- if you're next to the block (within 4 blocks), use `start_dig` to begin mining the block.
